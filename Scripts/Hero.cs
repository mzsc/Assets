...
public class Hero : MonoBehaviour {
    ...
    void Start() {
        ...
        InvokeRepeating("InstantiateBullet", 3f, 0.15f);
    }

    void Update() {
        if ((transform.position.y < -40) && (Time.time < (StartTime + 1.5f))) {
            transform.position = new Vector3(transform.position.x,
                Mathf.Lerp(-80f, -40f, (Time.time - StartTime) / 1.5f), transform.position.z);
            if (transform.position.y > -41) {
                transform.position = new Vector3(transform.position.x, -40, transform.position.z);
                ScriptPlaneWarControl.BoolPause = false;
            }           
        }
        ...
        if ((!ScriptPlaneWarControl.BoolGameOver) && (!ScriptPlaneWarControl.BoolPause)) {
            ...
            if (Input.GetMouseButton(0)) {
                ...
                //计算出战机横坐标
                HorizontalPos = 43.5f * ((mPos.x - Screen.width * 0.5f) / (Screen.width * 0.5f));
                //计算出战机纵坐标
    ...
    void InstantiateBullet() {
        ...
        if ((!ScriptPlaneWarControl.BoolGameOver) && (!ScriptPlaneWarControl.BoolPause)) {
            //在战机前面克隆子弹
            Instantiate(mBullet[0], new Vector3(transform.position.x, transform.position.y + 10f, 90f), new Quaternion(0, 0, 0, 0));
        }
    }
    ...
    void InstantiateBulletAdd() {
        ...
        if ((!ScriptPlaneWarControl.BoolGameOver) && (!ScriptPlaneWarControl.BoolPause)) {
            //在战机两侧克隆弹药1
            Instantiate(mBullet[1], new Vector3(transform.position.x - 4f, transform.position.y + 10f, 90f), new Quaternion(0, 0, 0, 0));
            Instantiate(mBullet[1], new Vector3(transform.position.x + 4f, transform.position.y + 10f, 90f), new Quaternion(0, 0, 0, 0));
