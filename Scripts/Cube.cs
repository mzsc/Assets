...
//组成俄罗斯方块的单个方块
    public string[] MyCubes;
//决定俄罗斯方块形状的数组
    private GameObject Console;
//场景中Console物体，控制台作用
    private Back BackScript;
//存储Back脚本
    private const float cWide = 0.5f;
//物理横坐标与背景数组y坐标转换差值
    private const float cHigh = 23.5f;
//物理纵坐标与背景数组x坐标转换差值
    private List<int> ListCheckX = new List<int>();
// 记录要检查的行
    ...
    void Update() {
        ...
        if ((!BackScript.Pause) && (!BackScript.GameOver)) {
            ...
            if (Input.GetKey(KeyCode.S)) {
                Time.fixedDeltaTime = 0.1f;//时间快10倍
            }
            //松开S键
            if (Input.GetKeyUp(KeyCode.S)) {
                int Multiple = BackScript.Integral / 1000 + 1;
        ...
        if ((!BackScript.Pause) && (!BackScript.GameOver)) {
            ...
            if (MovingDown()) {
                ...
                foreach (Transform Child in transform) {
                    ...
                    //检查是否已存在该元素
                    if (!ListCheckX.Contains(BackX)) {
                        ListCheckX.Add(BackX);//将要检查的行加入列表
                    }
                }
                transform.DetachChildren();//与子物体分离
                BackScript.CheckBacks(ListCheckX);//调用背景检查函数
